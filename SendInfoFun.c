...
int query_info(struct receive_info  *rec_info) {
    ...
    if (ret < 0) {
        ...
        recordLog(1, 2, strcat(response, "  the network maybe error(query_info)"));
        isLogin = 0;
        return -1;
    }
    ...
    if (result[0] != '\0') {
        ...
        for (i = 0; i < PARA_SIZE; i++) {
            ...
            if (strcmp(paraS[i].key, "cashier") == 0) {
                ...
                printf("no information from serviceNode to send here\n");
                recordLog(1, 2, "no information from serviceNode to send here");
                break;
            } else {
                printf("Receive error  :%s\n", temp_response);
                recordLog(1, 2, strcat(temp_response, " Receive error here"));
                return -1;
            }
        }
...
int request_ok(struct receive_info  *rec_info) {
    ...
    if (ret < 0) {
        printf("the network maybe error\n");
        recordLog(1, 2, strcat(response, "  the network maybe error(request_ok)"));
        isLogin = 0;
        return -1;
    }
    ...
    if (result[0] != '\0') {
        ...
        if (strcmp(result, "received ok") == 0) {
            ...
            recordLog(1, 2, "serviceNode has received modify ok here");
            //update sqlite here
            ...
            printf("serviceNode not received\n");
            recordLog(1, 2, "serviceNode not received");
        }
    }
...
int confirmed_ok(struct receive_info  *rec_info) {
    ...
    if (ret < 0) {
        printf("the network maybe error\n");
        recordLog(1, 2, strcat(response, "  the network maybe error(confirmed_ok)"));
        isLogin = 0;
        return -1;
    }
    ...
    if (result[0] != '\0') {
        ...
        if (strcmp(result, "confirmed ok") == 0) {
            ...
            recordLog(1, 2, strcat(response, "  received error(confirmed_ok)"));
        }
    }
